...
static void move_paddle_player2(int d){
	// if the S is pressed move paddle down
	if (d == 0) {
		
		if(paddle[0].y >= screen->h - paddle[1].h) {
		
			paddle[0].y = screen->h - paddle[1].h;
	...
	// if the W is pressed move paddle up
	if (d == 1) {
...
int main (int argc, char *args[]) {
	...
	while(quit == 0) {
		...
		if (keystate[SDL_SCANCODE_W]){
			move_paddle_player2(1);
		}
		if (keystate[SDL_SCANCODE_S]){
			move_paddle_player2(0);
		}
